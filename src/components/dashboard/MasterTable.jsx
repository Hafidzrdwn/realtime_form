export const MasterTable = ({ data, onDelete }) => {
  return (
    <div className="bg-gray-900 p-6 rounded-3xl border border-gray-800 shadow-xl mt-6">
          ...
          <tbody className="divide-y divide-gray-800/50">
            {data.map((item) => (
              <tr key={item.id} className="hover:bg-gray-800/40 transition-colors group">
                <td className="py-3.5 px-5 font-medium text-gray-200 whitespace-nowrap">{item.name}</td>
                <td className="py-3.5 px-5 text-center">{item.age}</td>
                <td className="py-3.5 px-5 text-center">
                  <span className="bg-blue-500/10 text-blue-400 px-3 py-1.5 rounded-full text-xs font-bold border border-blue-500/20 shadow-[0_0_10px_rgba(59,130,246,0.1)] inline-block min-w-[70px]">
                    {item.education}
                  </span>
                </td>
                ...
                <td className="py-3.5 px-5 text-center">
                  <button 
                    ...
                    className="text-red-400 hover:text-white bg-red-500/10 hover:bg-red-600 px-4 py-2 rounded-xl border border-red-500/20 transition-all text-xs font-bold shadow-sm opacity-50 group-hover:opacity-100 cursor-pointer"
                    title="Hapus Data"
                  >
                    Hapus
                  </button>
