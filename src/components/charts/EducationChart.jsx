  ...
  return (
    <div className="bg-gray-900 p-6 rounded-3xl border border-gray-800 shadow-xl flex flex-col">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-lg font-semibold text-gray-300">Tingkat Pendidikan</h2>
        {selectedEducation && (
          <button 
            onClick={() => setSelectedEducation(null)} 
            className="text-xs bg-gray-800 hover:bg-gray-700 text-gray-300 px-3 py-1.5 rounded-lg border border-gray-700 cursor-pointer transition-colors flex items-center gap-1"
          >
        ...
        <div className="h-64 relative animate-in fade-in duration-300">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              ...
              <Tooltip contentStyle={{ backgroundColor: '#1f2937', borderColor: '#374151', borderRadius: '0.5rem' }} itemStyle={{ color: '#f3f4f6' }} />
              <Legend verticalAlign="bottom" height={36} wrapperStyle={{ cursor: 'pointer' }} />
            </PieChart>
          </ResponsiveContainer>
        </div>
      ) : (
        <div className="flex-1 bg-gray-800/50 p-4 rounded-2xl border border-gray-700/50 animate-in fade-in duration-300 flex flex-col h-64">
